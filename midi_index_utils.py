# Documentation:
# Source code: https://github.com/robz/artemialis/blob/ecd1468e7c411ef8ad55e708309715046dbe67bd/MidiIndexUtils.py#L45
# http://www.somascape.org/midi/tech/mfile.html -> about the midi file
# https://web.mit.edu/music21/doc/moduleReference/moduleMidi.html

# Questions
# Is it supposed that only 1 event of the event_to_idx dictionary can occur at a time?

import math
import os
from music21 import midi
from file_utils import save_midi_file_from_midi_object


# Create the dictionary of possible events in the MIDI file (AKA the Vocabulary)
event_to_idx = {}
for i in range(128):
...
VOCABULARY_SIZE = len(idx_to_event)
...
def midiToIdxs(mf: midi.MidiFile):
    """
    Generates a list of indexes, where each index represents an event on the midi file mf

    Args:
        mf (TYPE): the midi file.
...
def main():
    ...
    dataset_path = "maestro_v3/1. initial songs"
    creations_path = "maestro_v3/3. creations"
    ...
    for path, subdirs, files in os.walk(dataset_path):
        ...
        for file in files:
            ...
            idxs = midiToIdxs(mf)
            new_midi_file, errors = idxsToMidi(idxs, verbose = True)
            save_midi_file_from_midi_object(new_midi_file, creations_path, "test.mid")
